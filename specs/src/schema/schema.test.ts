...
describe("Schema", () => {
  test("accepts schema prototype as schema interface creates schema interface from schema prototype", () => {
    ...
    assert<Equals<SchemaInterface<typeof Thing>, ThingType>>();

    expect(expandedSchema).toEqual(ThingSchema);
  });
});
